...
server {
    ...
    location /api/ {
        proxy_pass http://backend.mywebapp.local:8000/;  # backend is container name in Docker network
...
```
Note in nginx we'll proxy all api requests to the backend docker. Also note we name the backend as **backend.mywebapp.local**. The host naming style here is to make sure this hostname can be correcty resolved in AWS running service, we'll cover this part later.
...
services:
  ...
  backend:
    ...
    container_name: backend.mywebapp.local
    depends_on:
      postgres:
